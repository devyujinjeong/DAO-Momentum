...
> 약 8주간 진행되는 프로젝트이니만큼 기획단계에서부터 많은 노력이 들어갔는데, 기존 인사시스템의 기능을 이해하고 현실적으로 개발할 수 있는 레벨에서 개선점을 찾아내는 것이 쉽지 않았음. 또한 기획을 이해하고 해당 도메인의 흐름에 빠르게 적응하는 것도 개발자의 중요한 덕목임을 느낄 수 있었음 
<br>크게 배포, 챗봇, 공지사항(파일 업로드, 다운로드)를 맡아 구현 
<br> AWS에서 기존에 사용해보지 못했던 정말 다양한 서비스들을 이용해보며 식견을 넓힐 수 있었음. Cloudfront + S3 기반 파일 업로드 다운로드를 구현하며  네트워크 관련 기반지식에 대한 이해도(HTTP 통신, CORS) 증가함. 
<br> SSE+Kafka+Redis Pub/Sub를 활용하여 알림 서비스를 마이크로서비스형태로 구현(대용량 처리가 가능하도록). EKS를 통해 클러스터 기반 시스템을 처음 구현하게 되었는데, 실제 
<br> FASTAPI기반 RAG기반 사내 인사시스템 Q&A 챗봇을 구현하며, 전반적인 LangChain, LLM 등 AI 개발 생태계에 대해 느낄 수 있었음. 필요에 따라 학습하고 적용할 수 있는 다양한 기술들을 확인하며
